...
import { 
  Phone, Mail, Calendar, Send, CheckCircle, 
  MessageCircle, Sparkles, PhoneCall, FileText
} from 'lucide-react';
import GlassCard from './GlassCard';
import Carousel from './Carousel';

export default function ContactFormAndCarousel() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Dynamic import of canvas-confetti only upon successful submit to optimize bundle size
  ...
  return (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start w-full">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6 text-left w-full">
          ...
          <Carousel desktopSlidesToShow={1} className="w-full">
            ...
            <div className="flex items-start gap-4 p-5 rounded-2xl bg-white border border-border-custom shadow-sm h-full w-full">
              ...
              <div className="text-left">
                <span className="block text-[10px] text-body-text/80 uppercase font-bold">References</span>
                <span className="block text-sm font-bold text-heading mt-0.5">Academic & Professional References</span>
                <span className="block text-xs text-body-text/90 mt-1">Favorable academic and clinical references available upon request.</span>
              </div>
            </div>
          </Carousel>
        ...
        <div id="booking-form" className="lg:col-span-7 w-full text-left">
          <GlassCard hoverEffect={false} className="p-8 md:p-10 border border-white/60 bg-white/70 shadow-lg w-full">
            ...
            <div className="relative overflow-hidden min-h-[400px]">
              ...
              <form 
                ...
                className={`space-y-5 transition-all duration-500 ease-in-out ${
                  formStatus === 'success' ? 'opacity-0 scale-95 pointer-events-none absolute inset-0 -z-10' : 'opacity-100 scale-100'
                }`}
              >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className="w-full px-4 py-3 rounded-xl border border-border-custom bg-white/80 focus:border-secondary focus:bg-white outline-none transition-all text-sm text-heading font-medium focus:ring-4 focus:ring-primary/10 text-heading bg-transparent"
                  >
              ...
              {/* Success Content */}
              <div
                className={`bg-emerald-500/10 border border-emerald-500/20 p-8 rounded-2xl text-center flex flex-col items-center py-12 transition-all duration-500 ease-in-out absolute inset-0 ${
                  formStatus === 'success' ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-95 pointer-events-none -z-10'
                }`}
              >
      ...
      {/* Floating Sticky Bottom CTA for Mobile using high-performance CSS transforms */}
      <div
        className={`fixed bottom-0 left-0 right-0 z-40 bg-white/90 backdrop-blur-lg border-t border-border-custom p-4 flex sm:hidden items-center justify-between gap-3 shadow-[0_-8px_30px_rgba(15,23,42,0.06)] transition-all duration-300 ${
      ...
      {/* Floating Sticky WhatsApp Button (Visible above sm viewport) using CSS transitions */}
      <div className="fixed bottom-6 right-6 z-40 hidden sm:block">
        <button
          onClick={handleWhatsApp}
          className="w-14 h-14 rounded-full bg-emerald-500 hover:bg-emerald-600 text-white flex items-center justify-center shadow-xl shadow-emerald-500/20 border border-emerald-400/20 cursor-pointer hover:scale-110 hover:rotate-6 active:scale-95 transition-all duration-200"
